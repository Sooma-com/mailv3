...
require_once(__DIR__ . "/lib/nexus_client/client.php");
 ...
 */
class nexus extends rcube_plugin
{
    public $rc;
    public $client = null;
    public $config = null;
    ...
    public function init()
    {
        ...
        while (is_null($this->config) && $infinite_recursion_guard > 0 && isset($this->rc->config->get('nexus')[$config_domain])) {
            if (is_string($this->rc->config->get('nexus')[$config_domain])) {
                $config_domain = $this->rc->config->get('nexus')[$config_domain];
            } else {
                $this->config = $this->rc->config->get('nexus')[$config_domain];
            }
        ...
        $this->register_action('plugin.nexus_upgrade', [$this, 'nexus_upgrade']);
    }
    public function db() {
        if ($this->_db === null) {
    ...
    {
        $this->rc->output->add_label('nexus.upgrade');
        $this->rc->output->add_label('nexus.vat_invalid');
        $this->rc->output->add_label('nexus.vat_non_portuguese');
        $this->rc->output->add_label('nexus.customer_name_required');
        $this->rc->output->add_label('nexus.invalid_cellphone');
    }
    public function start(?string $mode = null): int {
        $this->rc->output->add_handlers([
                'nexus.product_selector' => [$this, 'product_selector'],
                'nexus.customer_info' => [$this, 'customer_info'],
                'nexus.payment_method' => [$this, 'payment_method'],
        ]);
        return 0;
    }
...
<fieldset id="nexus-payment-method">
 <span class="form-instructions"><?= $this->rc->gettext('nexus.select-payment-method', 'nexus'); ?></span>
 <div class="radio-group">
<?php if (is_null($last_payment_gateway) || $last_payment_gateway == 'stripe') { ?>
  <label class="monthly yearly">
   <input type="radio" name="payment_method" value="credit_card">
   <img height="40" src="/plugins/nexus/img/visa-mastercard.svg" alt="<?= $this->rc->gettext('credit_card', 'nexus'); ?>">
  </label>
<?php } ?>
<?php if (is_null($last_payment_gateway) || $last_payment_gateway == 'easypay') { ?>
  <label class="yearly">
   <input type="radio" name="payment_method" value="mb">
   <img height="40" src="/plugins/nexus/img/mb.svg" alt="<?= $this->rc->gettext('multibanco', 'nexus'); ?>">
  </label>
  <label class="yearly">
   <input type="radio" name="payment_method" value="mbway">
   <img height="40" src="/plugins/nexus/img/mbway.svg" alt="<?= $this->rc->gettext('mbway', 'nexus'); ?>">
  </label>
 </div>
 <label id="mbway-data"><?= $this->rc->gettext('mbway_telephone', 'nexus'); ?>
  <input required type="text" name="mbway_telephone" placeholder="91001002003">
  <button type="submit"><?= $this->rc->gettext('start_payment', 'nexus'); ?></button>
 </label>
<?php } ?>
</fieldset>
<?php
...
<fieldset id="nexus-customer-info">
 <span class="form-instructions"><?= $this->rc->gettext('nexus.fill-customer-info', 'nexus'); ?></span>
 <label><?= $this->rc->gettext('customer_vat_id', 'nexus'); ?>
  <input required type="text" name="customer_vat_id" value="<?= $customer_vat_id; ?>" placeholder="PT123123123">
  <span class="form-error"></span>
 </label>
 <label><?= $this->rc->gettext('customer_name', 'nexus'); ?>
...
<?php
    ...
    public function product_selector(array $attrib): string {
        ...
        if ($storage_product->get_metadata('profissional.on_expire.product_id')) {
            $template_products[] = [
                'nexus_id' => '',
                'name' => $this->rc->gettext('base_product_name', 'nexus'),
                'price' => [
                    'monthly' => 0,
                    'yearly' => 0,
                ]
...
<?php if ($template_product['action'] != 'none') { ?>
 <span class="nexus-product-option-action<?= $template_product['action'] ? sprintf(' action-%s', $template_product['action']) : ' no-action'; ?>"><?php switch ($template_product['action']) {
    case 'upgrade':
        print($this->rc->gettext('action.upgrade', 'nexus'));
        break;
    case 'downgrade':
        print($this->rc->gettext('action.downgrade', 'nexus'));
        break;
    case 'renew':
        print($this->rc->gettext('action.renew', 'nexus'));
        break;
 } ?>
 </span>
<?php } ?>
...
<fieldset id="nexus-price-selector">
 <span class="form-instructions"><?= $this->rc->gettext('nexus.select-price', 'nexus'); ?></span>
<?php
...
<?php       if (isset($template_product['price']['monthly'])) { ?>
  <label class="nexus-price-option monthly">
   ...
   <span class="nexus-price-option-name"><?= $this->rc->gettext('monthly_subscription', 'nexus'); ?></span>
 <?= sprintf('%0.2f', $template_product['price']['monthly']); ?>€ / <?= $this->rc->gettext('month', 'nexus'); ?>
  </label>
<?php        } ?>
<?php       if (isset($template_product['price']['yearly'])) { ?>
  <label class="nexus-price-option yearly">
   <input type="radio" name="price" value="yearly">
   <span class="nexus-price-option-name"><?= $this->rc->gettext('yearly_subscription', 'nexus'); ?></span>
 <?= sprintf('%0.2f', $template_product['price']['yearly'] / 12); ?>€ / <?= $this->rc->gettext('month', 'nexus'); ?>
   <span class="nexus-price-option-yearly-total"><?= $template_product['price']['yearly']; ?>€ / <?= $this->rc->gettext('year', 'nexus'); ?></span>
  </label>
<?php        } ?>
 </span>
 <?php  } ?>
...
<?php
    ...
    public function nexus_upgrade_get()
    {
        ...
        $this->rc->output->send('nexus.upgrade');
    }
}